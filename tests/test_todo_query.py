...
@pytest.mark.parametrize('case_info, done_status', [
    ('Кейс 46.1 - doneStatus = true', True),
    ('Кейс 46.2 - doneStatus = false', False)
])
def test_046_query_done_status(api_client, case_info, done_status):
    create_body = {
        'title': 'test title',
        'doneStatus': True,
    ...
    try:
        ...
        response = api_client.query(TODOS_PATH,
...
@pytest.mark.parametrize('case_info, done_status', [
    ('Кейс 47.1 - doneStatus = true', 'true'),
    ('Кейс 47.2 - doneStatus = false', 'false')
])
def test_047_query_done_status_jsonpath(api_client, case_info, done_status):
    create_body = {
        'title': 'test title',
        'doneStatus': True,
        'description': 'test description'}
...
@pytest.mark.parametrize('case_info, done_status', [
    ('Кейс 48.1 - doneStatus = true', True),
    ('Кейс 48.2 - doneStatus = false', False)
])
def test_048_query_done_status_json(api_client, case_info, done_status):
    query_body = {
        "filter": {
    ...
    try:
        ...
        assert_todo_item_matches(item=create_payload, expected=create_body)
        response = api_client.query(TODOS_PATH,
                                    headers={'Content-Type': 'application/vnd.thingifier.query'
                                                             '+json'},
                                    json=query_body)
        query_payload = response.json()
        todos = query_payload['todos']
