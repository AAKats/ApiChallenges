...
def test_046_query_done_status(api_client, _case_info, done_status):
...
def test_047_query_done_status_jsonpath(api_client, _case_info, done_status):
    create_body = {'title': 'test title', 'doneStatus': True, 'description': 'test description'}
    create_response = api_client.post(TODOS_PATH, json=create_body)
    create_payload = create_response.json()
    new_id = create_payload['id']
...
def test_048_query_done_status_json(api_client, _case_info, done_status):
